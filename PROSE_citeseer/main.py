...
from model import GCL, GCN_Classifer
...
class Experiment:
    ...
    def loss_gcl(self, model, graph_learner, features, anchor_adj, mi_type = 'learn'):
        ...
        # learned_adj, all_stage_adjs, cross_mi_loss, position_reg_loss = graph_learner(features, anchor_adj, pos_infos)
    ...
    def train(self, args):
        ...
        # random select nodes
        # n_nums = adj_original.shape[0]
        # n_indices = sorted(np.random.choice(range(n_nums), 5 * int(math.log(n_nums,2)), replace=False))
        ...
        for trial in range(args.ntrials):
            ...
            # adj_infos = copy.deepcopy(anchor_adj_raw)
            # pos_infos = adj_infos[n_indices, :][:, n_indices]
            ...
            if torch.cuda.is_available():
                ...
                # pos_infos = pos_infos.cuda()
            
            
            graph_learner = Stage_GNN_learner(2, features.shape[1], args.graph_learner_hidden_dim, args.k, args.sim_function, args.sparse,
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                # mi_loss, Adj, all_stage_adjs, cross_mi_loss, position_reg_loss = self.loss_gcl(model, graph_learner, features, anchor_adj, pos_infos, n_indices, args.head_tail_mi_type)
                mi_loss, Adj, prediction_adj = self.loss_gcl(model, graph_learner, features, anchor_adj, args.head_tail_mi_type)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-w_decay_gsl', type=float, default=5e-4)
    ...
    parser.add_argument('-lr_cl', type=float, default=0.001)
    ...
    parser.add_argument('-w_decay_cls', type=float, default=0.05)
